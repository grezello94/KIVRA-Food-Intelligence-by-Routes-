...
public sealed class KivraDbContext(DbContextOptions<KivraDbContext> options) : DbContext(options) {
 ...
 protected override void OnModelCreating(ModelBuilder b) { base.OnModelCreating(b);
  ...
  b.Entity<Label>().Property(x => x.ShelfLifeRuleSnapshot).HasMaxLength(100); b.Entity<PrintJob>().Property(x => x.Payload).HasColumnType("TEXT"); b.Entity<AuditLog>().Property(x => x.NewValues).HasColumnType("TEXT");
 }
}
