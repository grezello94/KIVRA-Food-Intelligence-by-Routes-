...
public sealed class PrinterOperations(KivraDbContext db, IEnumerable<ILabelPrinter> printers, ITsplGenerator tspl, TimeZoneInfo tz) {
 ...
 async Task<PrinterResult> TestCore(Guid id,CancellationToken ct) { var p=await db.Printers.FindAsync([id],ct)??throw new KeyNotFoundException("Printer not found."); if(p.Driver=="AndroidBridge"){var online=await db.BridgeDevices.AnyAsync(x=>x.Active&&x.RevokedAt==null&&x.LastSeenAt>DateTimeOffset.UtcNow.AddMinutes(-1),ct);return online?new(true):new(false,"No Android Print Bridge is online.");}return await Resolve(p).TestConnectionAsync(p,ct); }
 public async Task<PrinterResult> TestPrintAsync(Guid printerId,CancellationToken ct) { var p=await db.Printers.FindAsync([printerId],ct)??throw new KeyNotFoundException("Printer not found.");if(p.Driver=="AndroidBridge")return new(false,"Run Test printer in the Android Print Bridge app.");var sample=new Label { LabelCode="TEST-0001",ItemNameSnapshot="KIVRA TEST LABEL",CategorySnapshot="System",ClassificationSnapshot=Classification.NotApplicable,DateTerminologySnapshot="TESTED",OperationalDateTime=DateTimeOffset.UtcNow,ExpiryDateTime=DateTimeOffset.UtcNow,ShelfLifeRuleSnapshot="Test",StorageLocationSnapshot="PRINTER TEST" }; return await Resolve(p).PrintAsync(p,tspl.Generate(sample,p,tz),ct); }
}
